...
def record_frame(recorder, rgb, depth, cam_pos, cam_mat, link_poses, ee_pose):
    ...
    q = R.from_matrix(cam_mat_cv).as_quat()
...
def main():
    ...
    cabinet_urdf = os.path.join(base_dir, "44817", "mobility.urdf")
    cabinet = loader.load(cabinet_urdf)
    # 44817 bounding box z: [-0.416327, 0.426529]，底部贴地放置
    cabinet.set_pose(sapien.Pose(p=[1.367079, 0.197510, 0.716]))
    ...
    for j in arm_joints:
        j.set_drive_property(stiffness=800, damping=70)
    for j in gripper_joints:
        j.set_drive_property(stiffness=5000, damping=2000, force_limit=2000000.0)

    for j in gripper_joints:
        j.set_armature(np.ones(j.dof, dtype=np.float32) * 1.0)
    ...
    # 相机目标 z 根据 44817 柜子中心高度调整（pose_z=0.416，中心≈0.416）
    cam_eye, cam_target = np.array([-1.14, -0.09, 1.2]), np.array([1.367079, 0.197510, 0.72])
        ...
        while True:
            ...
            if comm_thread.obs_queue.empty():
                ...
                if recorder is not None:
                    ...
                    if now - recorder['last_save_time'] >= 0.1:
